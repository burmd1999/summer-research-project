...
allfiles_coh_psd = cell(length(eegdatasets_list), 2);
allfiles_coh_psd(:, 1) = eegdatasets_list(:, 1);
...
parfor i = 1:length(eegdatasets_list)
       ...
       file_coh_psd = cell(length(all_cond), 3);
       file_coh_psd(:, 1) = all_cond(:, 1);
       file_coh_psd(:, 2) = coherence(:, 1);
       file_coh_psd(:, 3) = powerspd(:, 1);
       allfiles_coh_psd{i, 2} = file_coh_psd;
end    
